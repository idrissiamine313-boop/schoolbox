...
function IconStar({ size = 16, color = ORANGE, filled = false }: any) { return <Svg width={size} height={size} viewBox="0 0 24 24" fill={filled ? color : 'none'}><Path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" stroke={color} strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" /></Svg>; }
function IconX({ size = 14, color = RED }: any) { return <Svg width={size} height={size} viewBox="0 0 24 24" fill="none"><Line x1="18" y1="6" x2="6" y2="18" stroke={color} strokeWidth={2.5} strokeLinecap="round" /><Line x1="6" y1="6" x2="18" y2="18" stroke={color} strokeWidth={2.5} strokeLinecap="round" /></Svg>; }
...
export default function AdminProducts() {
  ...
  const [fImgUploading, setFImgUploading] = useState(false);
  ...
  async function loadData() {
    ...
    const [sc, lv, fo, ca, an] = await Promise.all([
      supabase.from('schools').select('id,name,abbreviation,logo_url').eq('is_active', true).order('name'),
      ...
      supabase.from('announcements').select('id,title,body,image_url,school_ids,level_ids,level_names,branches,type,is_active,created_at').order('created_at', { ascending: false }),
    ]);
    setSchools(sc.data || []);
  ...
  function getAvailBranches(selLevelNames: string[]) {
    const bs = new Set<string>();
    selLevelNames.forEach(name => {
      if (NIVEAU_BRANCHES[name]) NIVEAU_BRANCHES[name].forEach((b: string) => bs.add(b));
    });
  ...
  async function addCImage() {
    try {
      const perm = await ImagePicker.requestMediaLibraryPermissionsAsync();
      if (!perm.granted) { Alert.alert('Permission requise', 'Autorisez l\'accès aux photos'); return; }
      const res = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ImagePicker.MediaTypeOptions.Images, quality: 0.75, allowsMultipleSelection: true });
      if (!res.canceled && res.assets.length > 0) {
        setCImgUploading(true);
        const urls: string[] = [];
        for (const asset of res.assets) {
          const url = await uploadFile('catalogue', asset.uri, 'image');
          urls.push(url);
        }
        setCImages(p => [...p, ...urls]);
        setCImgUploading(false);
      }
    } catch (e: any) { Alert.alert('Erreur upload', e.message); setCImgUploading(false); }
  }
  ...
  // ── FOURNITURE CRUD
  function openAddF() {
    setEditF(null); setFName('');
    setFSchools(selectedSchool ? [selectedSchool.id] : []);
    setFLevels([]); setFBranches([]);
    setFWrapping('0'); setFItems([]); setFItemName(''); setFItemPrice('');
    setFItemImage(null); setFPdf(null); setFPdfName(''); setFImage(null);
    setShowFModal(true);
  }

  function openEditF(f: any) {
    setEditF(f); setFName(f.name || '');
    setFSchools(f.school_ids || []); setFLevels(f.level_names || f.level_ids || []); setFBranches(f.branches || []);
    ...
    setFImage(f.image_url || null);
    setFItemName(''); setFItemPrice(''); setFItemImage(null);
    setShowFModal(true);
  }

  function removeFItem(index: number) {
    ...
    if (item.id) {
      Alert.alert('Supprimer cet article ?', `"${item.name}"`, [
        { text: 'Annuler', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setLoading(true);
            const { error } = await supabase.from('fourniture_items').delete().eq('id', item.id);
            if (error) { Alert.alert('Erreur', error.message); setLoading(false); return; }
            setFItems(p => p.filter((_, j) => j !== index));
            setLoading(false);
          }
        },
      ]);
  ...
  async function saveF() {
    ...
    try {
      const payload: any = {
        ...
        level_names: fLevels,
        branches: fBranches,
        wrapping_price: parseFloat(fWrapping) || 0,
        pdf_url: fPdf || null,
      ...
      if (editF) {
        ...
        if (error) throw error;
        const newItems = fItems.filter(it => !it.id);
        if (newItems.length > 0) {
          const { error: ie } = await supabase.from('fourniture_items').insert(
            newItems.map(it => ({ fourniture_id: fid, name: it.name, price: it.price, image_url: it.image_url || null }))
          );
          if (ie) throw ie;
        }
      } else {
        ...
        fid = data.id;
        const { error: ie } = await supabase.from('fourniture_items').insert(
          fItems.map(it => ({ fourniture_id: fid, name: it.name, price: it.price, image_url: it.image_url || null }))
        );
        if (ie) throw ie;
      }
  ...
  function openAddC() {
    ...
    setCSchools(selectedSchool ? [selectedSchool.id] : []);
    setCLevels([]); setCBranches([]);
    setCImages([]); setCFavoris(false); setCCategory('general');
    setShowCModal(true);
  }

  function openEditC(p: any) {
    setEditC(p); setCName(p.name || ''); setCPrice(String(p.price || ''));
    setCDesc(p.description || ''); setCSchools(p.school_ids || []); setCLevels(p.level_names || p.level_ids || []);
    setCBranches(p.branches || []);
    const imgs = p.image_urls?.length ? p.image_urls : (p.image_url ? [p.image_url] : []);
    setCImages(imgs);
    setCFavoris(p.is_favoris || false);
    setCCategory(p.category || 'general');
    setShowCModal(true);
  }
  ...
  function openAddA() {
    ...
    setASchools(selectedSchool ? [selectedSchool.id] : []);
    setALevels([]); setABranches([]);
  ...
  function openEditA(a: any) {
    ...
    setASchools(a.school_ids || []); setALevels(a.level_names || a.level_ids || []);
  ...
  async function saveA() {
    ...
    try {
      const payload: any = {
        title: aTitle.trim(), body: aContent || null,
        school_ids: aSchools, level_ids: [], level_names: aLevels,
        branches: aBranches, type: aType, is_active: true, image_url: aImage || null
      };
      if (editA) {
  ...
  async function toggleFavoris(p: any) {
    await supabase.from('products').update({ is_favoris: !p.is_favoris }).eq('id', p.id);
    loadData();
  }
  ...
  function LevelSel({ sel, setSel, color }: any) {
    const displayLevels = (levels || []).filter((l: any, i: number, arr: any[]) =>
      arr.findIndex((x: any) => x.name === l.name) === i
    );
    const allSel = displayLevels.every((l: any) => sel.includes(l.name));
    return (
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        <View style={{ flexDirection: 'row', gap: 8, paddingVertical: 4 }}>
          <TouchableOpacity
            style={[s.chip, allSel && { backgroundColor: color, borderColor: color }]}
            onPress={() => { if (allSel) setSel([]); else setSel(displayLevels.map((l: any) => l.name)); }}>
            <IconCheck size={11} color={allSel ? 'white' : color} />
            <Text style={[s.chipTxt, allSel && { color: 'white' }]}>Tous</Text>
          </TouchableOpacity>
          {displayLevels.map((lv: any) => (
            <TouchableOpacity key={lv.id}
              style={[s.chip, sel.includes(lv.name) && { backgroundColor: color, borderColor: color }]}
              onPress={() => setSel((p: string[]) => p.includes(lv.name) ? p.filter(x => x !== lv.name) : [...p, lv.name])}>
              <Text style={[s.chipTxt, sel.includes(lv.name) && { color: 'white' }]}>{lv.name}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </ScrollView>
  ...
  function BranchSel({ selLvIds, sel, setSel, color }: any) {
    const brs = getAvailBranches(selLvIds);
    if (brs.length === 0) return null;
    const allSel = sel.length === brs.length && brs.length > 0;
    return (
      <>
        <Text style={s.lbl}>BRANCHE</Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={{ flexDirection: 'row', gap: 8, paddingVertical: 4 }}>
            <TouchableOpacity style={[s.chip, allSel && { backgroundColor: color, borderColor: color }]} onPress={() => setSel(allSel ? [] : brs)}>
              <IconCheck size={11} color={allSel ? 'white' : color} /><Text style={[s.chipTxt, allSel && { color: 'white' }]}>Tous</Text>
            </TouchableOpacity>
            {brs.map((b: string) => (
    ...
    <View style={s.container}>
      ...
      <View style={[s.header, { backgroundColor: activeColor }]}>
        ...
        <View style={s.headerRow}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={s.headerTitle}>{selectedSchool ? selectedSchool.name : 'Produits & Annonces'}</Text>
            <Text style={s.headerSub}>{selectedSchool ? activeTab : 'Choisissez une école'}</Text>
          </View>
          {selectedSchool && (
            <TouchableOpacity style={s.addBtn} onPress={() => {
              if (activeTab === 'fournitures') openAddF();
              else if (activeTab === 'catalogue') openAddC();
              else openAddA();
            }}><IconPlus /></TouchableOpacity>
          )}
        </View>
          ...
          {TABS.map(tab => {
            ...
            return (
              <TouchableOpacity key={tab.key}
                style={[s.tab, active && { backgroundColor: 'rgba(255,255,255,0.25)' }]}
                onPress={() => { setActiveTab(tab.key); setSelectedSchool(null); }}>
      ...
      <ScrollView
        ...
        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={loadData} tintColor={activeColor} />}
        showsVerticalScrollIndicator={false}>

        {/* ── قائمة المدارس ── */}
        {!selectedSchool && (
          <View>
            <Text style={[s.secTitle, { color: activeColor }]}>🏫  CHOISIR UNE ÉCOLE</Text>
            {schools.map(sc => {
              const count = activeTab === 'fournitures'
                ? fournitures.filter(f => f.school_ids?.includes(sc.id)).length
                : activeTab === 'catalogue'
                ? catalogue.filter(p => p.school_ids?.includes(sc.id)).length
                : annonces.filter(a => a.school_ids?.includes(sc.id)).length;
              return (
                <TouchableOpacity key={sc.id} style={s.schoolCard}
                  onPress={() => setSelectedSchool(sc)} activeOpacity={0.85}>
                  <View style={[s.schoolIcon, { backgroundColor: activeColor + '15' }]}>
  {sc.logo_url
    ? <Image source={{ uri: sc.logo_url }} style={{ width: 52, height: 52, borderRadius: 16 }} resizeMode="contain" />
    : <Text style={{ fontSize: 24 }}>🏫</Text>
  }
</View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.schoolName}>{sc.name}</Text>
                    {sc.abbreviation && <Text style={s.schoolAbbr}>{sc.abbreviation}</Text>}
                  </View>
                  <View style={[s.schoolCount, { backgroundColor: activeColor + '15' }]}>
                    <Text style={[s.schoolCountTxt, { color: activeColor }]}>{count}</Text>
                  </View>
                </TouchableOpacity>
              );
            })}
        ...
        {/* ── FOURNITURES ── */}
        {selectedSchool && activeTab === 'fournitures' && (() => {
          const filtered = fournitures.filter(f => f.school_ids?.includes(selectedSchool.id));
          return filtered.length === 0
            ? <View style={s.empty}>
                <View style={[s.emptyIcon, { backgroundColor: GREEN + '18' }]}><IconBook size={40} color={GREEN} /></View>
                <Text style={s.emptyTxt}>Aucune fourniture pour cette école</Text>
                <TouchableOpacity style={[s.emptyBtn, { backgroundColor: GREEN }]} onPress={openAddF}>
                  <IconPlus /><Text style={s.emptyBtnTxt}>Ajouter</Text>
                </TouchableOpacity>
              </View>
            : <>{filtered.map(f => (
                <View key={f.id} style={s.card}>
                  <View style={s.cardHeader}>
                    {f.image_url
                      ? <Image source={{ uri: f.image_url }} style={s.prodImg} />
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {(f.level_names?.length > 0 || f.level_ids?.length > 0) && (
                        <Text style={s.cardMeta2}>
                          {f.level_names?.length > 0 ? f.level_names.join(', ') : getNames(f.level_ids, levels)}
                          {f.branches?.length > 0 ? ` • ${f.branches.join(', ')}` : ''}
                        </Text>
                      )}
                    </View>
                    ...
                    {f.items?.slice(0, 3).map((item: any, i: number) => (
                      <View key={i} style={s.itemRow}>
                        {item.image_url
                          ...
                          : <View style={[s.itemImg, { backgroundColor: '#f0fdf4', justifyContent: 'center', alignItems: 'center' }]}><IconImage size={14} color={GREEN} /></View>
                        }
                  ...
                  <View style={s.cardFooter}>
                    <View style={{ flexDirection: 'row', gap: 6, flexWrap: 'wrap', flex: 1 }}>
                      ...
                      {f.wrapping_price > 0 && <View style={[s.badge, { backgroundColor: ORANGE + '15' }]}><IconShield size={10} /><Text style={[s.badgeTxt, { color: ORANGE }]}>+{f.wrapping_price} MAD</Text></View>}
                      {f.pdf_url && (
        ...
        {/* ── CATALOGUE ── */}
        {selectedSchool && activeTab === 'catalogue' && (() => {
          const filtered = catalogue.filter(p => p.school_ids?.includes(selectedSchool.id));
          return filtered.length === 0
            ? <View style={s.empty}>
                <View style={[s.emptyIcon, { backgroundColor: PURPLE + '18' }]}><IconBox size={40} color={PURPLE} /></View>
                <Text style={s.emptyTxt}>Aucun produit pour cette école</Text>
                <TouchableOpacity style={[s.emptyBtn, { backgroundColor: PURPLE }]} onPress={openAddC}>
                  <IconPlus /><Text style={s.emptyBtnTxt}>Ajouter</Text>
                </TouchableOpacity>
              </View>
            : <>{filtered.map(p => {
                const imgs: string[] = p.image_urls?.length ? p.image_urls : (p.image_url ? [p.image_url] : []);
                return (
                  <View key={p.id} style={s.card}>
                        ...
                        <View style={{ flexDirection: 'row', gap: 8 }}>
                          {imgs.map((img, i) => (
                            <View key={i} style={{ position: 'relative' }}>
                              ...
                              {i === 0 && <View style={s.mainImgBadge}><Text style={s.mainImgTxt}>MAIN</Text></View>}
                            </View>
                          ))}
                        </View>
                      </ScrollView>
                    ...
                    <View style={s.cardHeader}>
                      {imgs.length === 0 && (
                        <View style={[s.prodImg, { backgroundColor: PURPLE + '15', justifyContent: 'center', alignItems: 'center' }]}><IconBox size={28} color={PURPLE} /></View>
                      )}
                      <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                          <Text style={s.cardTitle}>{p.name}</Text>
                          {p.is_favoris && <Text style={{ fontSize: 14 }}>⭐</Text>}
                        </View>
                        ...
                        {(p.level_names?.length > 0 || p.level_ids?.length > 0) && (
                          <Text style={s.cardMeta2}>
                            {p.level_names?.length > 0 ? p.level_names.join(', ') : getNames(p.level_ids, levels)}
                          </Text>
                        )}
                      </View>
                      <View style={s.cardActions}>
                        <TouchableOpacity style={[s.actionBtn, { backgroundColor: p.is_favoris ? ORANGE + '20' : '#f7f8fc' }]} onPress={() => toggleFavoris(p)}>
                          <IconStar size={14} color={ORANGE} filled={p.is_favoris} />
                        </TouchableOpacity>
                    ...
                    <View style={s.cardFooter}>
                      <View style={{ flexDirection: 'row', gap: 6 }}>
                        <View style={[s.badge, { backgroundColor: p.is_active ? '#dcfce7' : '#f3f4f6' }]}>
                          <Text style={[s.badgeTxt, { color: p.is_active ? GREEN : '#9ca3af' }]}>{p.is_active ? 'Actif' : 'Inactif'}</Text>
                        </View>
                        {imgs.length > 1 && (
                          <View style={[s.badge, { backgroundColor: PURPLE + '15' }]}>
                            <Text style={[s.badgeTxt, { color: PURPLE }]}>📷 {imgs.length} photos</Text>
                          </View>
                        )}
                      </View>
        ...
        {/* ── ANNONCES ── */}
        {selectedSchool && activeTab === 'annonces' && (() => {
          const filtered = annonces.filter(a => a.school_ids?.includes(selectedSchool.id));
          return filtered.length === 0
            ? <View style={s.empty}>
                <View style={[s.emptyIcon, { backgroundColor: ORANGE + '18' }]}><IconBell size={40} color={ORANGE} /></View>
                <Text style={s.emptyTxt}>Aucune annonce pour cette école</Text>
                <TouchableOpacity style={[s.emptyBtn, { backgroundColor: ORANGE }]} onPress={openAddA}>
                  <IconPlus /><Text style={s.emptyBtnTxt}>Ajouter</Text>
                </TouchableOpacity>
              </View>
            : <>{filtered.map(a => (
                <View key={a.id} style={s.card}>
                  <View style={s.cardHeader}>
                    {a.image_url
                      ? <Image source={{ uri: a.image_url }} style={s.prodImg} />
                      : <View style={[s.prodImg, { backgroundColor: ORANGE + '15', justifyContent: 'center', alignItems: 'center' }]}><IconBell size={28} color={ORANGE} /></View>
                    }
                    <View style={{ flex: 1 }}>
                      ...
                      {(a.level_names?.length > 0 || a.level_ids?.length > 0) && (
                        <Text style={s.cardMeta2}>
                          {a.level_names?.length > 0 ? a.level_names.join(', ') : getNames(a.level_ids, levels)}
                        </Text>
                      )}
                    </View>
                    <View style={s.cardActions}>
          ...
          <View style={[s.sheet, { maxHeight: '85%' }]}>
            ...
            <View style={{ flex: 1, backgroundColor: '#f7f8fc', borderRadius: 16, padding: 20, alignItems: 'center', justifyContent: 'center', gap: 16 }}>
              ...
              <TouchableOpacity style={[s.saveBtn, { backgroundColor: RED, marginTop: 8, paddingHorizontal: 32 }]}
                onPress={() => { setShowPdfModal(false); Alert.alert('PDF', pdfUrl); }}>
        ...
        <View style={s.overlay}>
          <View style={s.sheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <Text style={s.lbl}>PHOTO DE LA FOURNITURE</Text>
              <TouchableOpacity style={[s.imagePicker, fImage && { borderStyle: 'solid', borderColor: GREEN }]}
                onPress={() => pickImage(setFImage, setFImgUploading, 'fournitures')} disabled={fImgUploading}>
                {fImgUploading ? <ActivityIndicator color={GREEN} size="large" />
                  : fImage ? <Image source={{ uri: fImage }} style={s.imagePreview} />
                    : <><IconImage size={40} color="#d1d5db" /><Text style={s.imagePickerTxt}>Appuyer pour ajouter une photo</Text></>}
              </TouchableOpacity>
              {fImage && (
                <TouchableOpacity onPress={() => setFImage(null)} style={s.removeImgBtn}>
                  <IconX size={12} color={RED} /><Text style={s.removeImgTxt}>Supprimer la photo</Text>
                </TouchableOpacity>
              )}
                ...
                <View style={{ marginTop: 8, gap: 6 }}>
                  {fItems.map((item, i) => (
                    <View key={i} style={s.fItemRow}>
                      {item.image_url
                        ? <Image source={{ uri: item.image_url }} style={s.fItemImg} />
                        : <View style={[s.fItemImg, { backgroundColor: '#f0fdf4', justifyContent: 'center', alignItems: 'center' }]}><IconImage size={14} color={GREEN} /></View>
                      }
                      <Text style={s.fItemName} numberOfLines={1}>{item.name}</Text>
                      <Text style={s.fItemPrice}>{Number(item.price).toFixed(2)}</Text>
                      <TouchableOpacity onPress={() => removeFItem(i)}><IconTrash size={14} /></TouchableOpacity>
                    </View>
                  ))}
        ...
        <View style={s.overlay}>
          <View style={s.sheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <Text style={s.lbl}>PHOTOS DU PRODUIT ({cImages.length}/8)</Text>
              {cImages.length > 0 && (
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
                  <View style={{ flexDirection: 'row', gap: 8, paddingVertical: 4 }}>
                    {cImages.map((img, i) => (
                      <View key={i} style={{ position: 'relative' }}>
                        <Image source={{ uri: img }} style={s.multiImgPreview} />
                        {i === 0 && <View style={s.mainImgBadge}><Text style={s.mainImgTxt}>MAIN</Text></View>}
                        <TouchableOpacity style={s.removeImgCircle} onPress={() => setCImages(p => p.filter((_, j) => j !== i))}>
                          <IconX size={10} color="white" />
                        </TouchableOpacity>
                      </View>
                    ))}
                    {cImages.length < 8 && (
                      <TouchableOpacity style={s.addImgBox} onPress={addCImage} disabled={cImgUploading}>
                        {cImgUploading ? <ActivityIndicator color={PURPLE} size="small" />
                          : <><IconPlus size={20} color={PURPLE} /><Text style={s.addImgTxt}>Ajouter</Text></>}
                      </TouchableOpacity>
                    )}
                  </View>
                </ScrollView>
              ...
              {cImages.length === 0 && (
                <TouchableOpacity style={s.imagePicker} onPress={addCImage} disabled={cImgUploading}>
                  {cImgUploading ? <ActivityIndicator color={PURPLE} size="large" />
                    : <><IconImage size={40} color="#d1d5db" /><Text style={s.imagePickerTxt}>Appuyer pour ajouter des photos</Text><Text style={{ fontSize: 11, color: '#9ca3af' }}>Jusqu'à 8 photos</Text></>}
                </TouchableOpacity>
              )}
              <TouchableOpacity style={[s.favorisBtn, cFavoris && s.favorisBtnActive]} onPress={() => setCFavoris(p => !p)}>
                <IconStar size={18} color={cFavoris ? 'white' : ORANGE} filled={cFavoris} />
                <View style={{ flex: 1 }}>
                  <Text style={[s.favorisTxt, cFavoris && { color: 'white' }]}>Produit Favori ⭐</Text>
                  <Text style={[s.favorisSub, cFavoris && { color: 'rgba(255,255,255,0.7)' }]}>Apparaît en priorité sur l'accueil</Text>
                </View>
                <View style={[s.favorisToggle, cFavoris && { backgroundColor: 'rgba(255,255,255,0.3)' }]}>
                  <Text style={{ fontSize: 12, fontWeight: '900', color: cFavoris ? 'white' : ORANGE }}>{cFavoris ? 'OUI' : 'NON'}</Text>
                </View>
              </TouchableOpacity>
              <Text style={s.lbl}>NOM *</Text>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                <View style={{ flexDirection: 'row', gap: 8, paddingVertical: 4 }}>
                  {['general', 'uniforms', 'books', 'supplies', 'canteen'].map(cat => (
                    <TouchableOpacity key={cat} style={[s.chip, cCategory === cat && { backgroundColor: PURPLE, borderColor: PURPLE }]} onPress={() => setCCategory(cat)}>
                      <Text style={[s.chipTxt, cCategory === cat && { color: 'white' }]}>
                        {cat === 'general' ? '🏷️ Général' : cat === 'uniforms' ? '👕 Uniforme' : cat === 'books' ? '📚 Livres' : cat === 'supplies' ? '✏️ Fournitures' : '🍽️ Cantine'}
                      </Text>
                    </TouchableOpacity>
                  ))}
                </View>
        ...
        <View style={s.overlay}>
          <View style={s.sheet}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              ...
              <TouchableOpacity style={[s.imagePicker, { height: 150 }, aImage && { borderStyle: 'solid', borderColor: ORANGE }]}
                onPress={() => pickImage(setAImage, setAImgUploading, 'annonces')} disabled={aImgUploading}>
...
const s = StyleSheet.create({
  ...
  scroll: { padding: 16 },
  secTitle: { fontSize: 11, fontWeight: '900', letterSpacing: 1.5, marginBottom: 14 },
  schoolCard: { backgroundColor: 'white', borderRadius: 18, padding: 16, marginBottom: 10, flexDirection: 'row', alignItems: 'center', gap: 14, shadowColor: NAV, shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2, borderWidth: 1, borderColor: 'rgba(15,35,86,0.06)' },
  schoolIcon: { width: 52, height: 52, borderRadius: 16, justifyContent: 'center', alignItems: 'center' },
  schoolName: { fontSize: 15, fontWeight: '900', color: NAV },
  schoolAbbr: { fontSize: 11, color: '#9ca3af', marginTop: 2, fontWeight: '600' },
  schoolCount: { width: 36, height: 36, borderRadius: 12, justifyContent: 'center', alignItems: 'center' },
  schoolCountTxt: { fontSize: 16, fontWeight: '900' },
  ...
  carouselImg: { width: 110, height: 110, borderRadius: 14 },
  multiImgPreview: { width: 90, height: 90, borderRadius: 14 },
  mainImgBadge: { position: 'absolute', bottom: 6, left: 6, backgroundColor: PURPLE, borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
  mainImgTxt: { fontSize: 8, fontWeight: '900', color: 'white', letterSpacing: 0.5 },
  removeImgCircle: { position: 'absolute', top: 4, right: 4, width: 22, height: 22, borderRadius: 11, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center' },
  addImgBox: { width: 90, height: 90, borderRadius: 14, backgroundColor: PURPLE + '10', borderWidth: 2, borderColor: PURPLE + '30', borderStyle: 'dashed', justifyContent: 'center', alignItems: 'center', gap: 4 },
  addImgTxt: { fontSize: 10, color: PURPLE, fontWeight: '800' },
  removeImgBtn: { flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8, paddingVertical: 6 },
  removeImgTxt: { fontSize: 12, color: RED, fontWeight: '700' },
  favorisBtn: { flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: ORANGE + '10', borderRadius: 16, padding: 14, marginTop: 16, borderWidth: 1.5, borderColor: ORANGE + '30' },
  favorisBtnActive: { backgroundColor: ORANGE, borderColor: ORANGE },
  favorisTxt: { fontSize: 14, fontWeight: '900', color: ORANGE },
  favorisSub: { fontSize: 11, color: ORANGE + 'aa', fontWeight: '600', marginTop: 1 },
  favorisToggle: { backgroundColor: ORANGE + '20', borderRadius: 10, paddingHorizontal: 10, paddingVertical: 5 },
});